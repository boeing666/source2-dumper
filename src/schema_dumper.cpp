...
int DumpAll(const fs::path& binDir, const fs::path& moduleDir, const fs::path& outDir) {
	...
	const ScriptApi scriptApi = CollectScriptApi(mods);
	...
	WriteJson(outDir, modules, known, network, stateChanged, pulseIO, scriptApi, convars, concommands, events, patch, serverVer);
