...
// ---- JSON shapes (glaze reflects these; MUST be external linkage — not in an anonymous namespace) ----
struct JField {
	std::string name;
	std::string type;
	int32_t offset = 0;
	int32_t size = 0;
	int32_t align = -1;   // -1 = unspecified (SDK 0xFF)
	bool networked = false;
	int32_t bits = 0;     // 0 = not a bitfield
...
// one datamap row: kind = input|output|keyfield|field; name = key/external or field name;
// target = handler (inputs) or the schema member it writes (outputs/keyfields/fields), "" if none.
struct JDatamap {
	std::string kind;
	std::string name;
	std::string target;
	std::string type;   // FIELD_* type name
};
...
struct JClass {
	...
	std::vector<std::string> refs;   // referenced by
};
...
// compact global index (search / home / sort / filters), all types across scopes
struct JIndexEntry {
	std::string name;
	std::string kind;    // class | struct | enum
	std::string scope;
	std::string file;    // schema.<scope>.json
	std::string parent;  // immediate base (for the inheritance chain)
	bool entity = false;
	int32_t nf = 0;      // field / enumerator count
	int32_t depth = 0;   // inheritance depth
	int32_t ni = 0, no = 0, nk = 0;  // datamap inputs / outputs / keyfields
	std::string fields;   // space-joined field/enumerator names
	std::string offsets;  // space-joined hex field offsets (index-aligned with fields)
};
...
// ---- helpers ported from the old html_writer (data extraction, not markup) --------------------------
...
static void CollectRefs(CSchemaClassInfo* c, const std::vector<const SchemaClassFieldData_t*>& fields,
	...
	auto scan = [&](std::string_view s) {
		for (size_t i = 0; i < s.size();) {
			...
			if (known.count(id)) { local.insert(std::move(id)); }
			i = j;
		}
	};
...
// datamap rows + input/output/keyfield counts (typedescription_t flags verified against CBaseEntity)
static void BuildDatamap(CSchemaClassInfo* c, std::vector<JDatamap>& out, int& ni, int& no, int& nk) {
	...
	// flattened offset -> schema member (over this class + bases), so a datamap entry names the field it writes
...
static void WriteFile(const fs::path& path, const T& obj) {
	...
	if (auto ec = glz::write_json(obj, buf); ec) {
		std::fprintf(stderr, "json: write error for %s\n", path.string().c_str());
		return;
	}
...
static JClass BuildClass(const ClassRec& rec, const std::unordered_set<std::string>& network,
	...
	for (const SchemaClassFieldData_t* f : rec.fields) {
		...
		jf.networked = f->m_pszName && network.count(jc.name + "::" + f->m_pszName) != 0;
...
void WriteJson(const fs::path& outDir,
	...
	for (const Module& m : modules) {
		...
		for (const ClassRec& rec : m.classes) {
			...
			for (const JField& f : jc.fields) {
				...
				ix.fields += f.name;
				char buf[16];
				std::snprintf(buf, sizeof(buf), "0x%x", (unsigned)f.offset);
				ix.offsets += buf;
			}
	...
	std::printf("json: %d classes / %d enums / %zu convars / %zu concommands -> %s\n",
	            meta.classes, meta.enums, convars.size(), concommands.size(), dir.string().c_str());
}
