#pragma once

class CDOTA_Modifier_AghsFort_RestorativeFlower : public CDOTA_Buff /*0x0*/  // sizeof 0x1A98, align 0xFF [vtable] (client)
{
public:
    char _pad_0000[0x1A78]; // offset 0x0
    int32 radius; // offset 0x1A78, size 0x4, align 4
    int32 heal; // offset 0x1A7C, size 0x4, align 4
    int32 vision_radius; // offset 0x1A80, size 0x4, align 4
    float32 vision_duration; // offset 0x1A84, size 0x4, align 4
    int32 model_scale; // offset 0x1A88, size 0x4, align 4
    bool bExploding; // offset 0x1A8C, size 0x1, align 1
    char _pad_1A8D[0x3]; // offset 0x1A8D
    float32 stun_duration; // offset 0x1A90, size 0x4, align 4
    ParticleIndex_t m_nFXAmbient; // offset 0x1A94, size 0x4, align 255
};
